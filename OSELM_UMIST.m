...
%load cifar_kmeans_train1
%load cifar_kmeans_train2
%load cifar_kmeans_train3
...
%P=train(:,1:(size(train,2)-1));
%T=train(:,size(train,2));
%clear train


 rand_sequence=randperm(size(P,1));
        temp_P=P;
        temp_T=T;
        clear P;
        clear T;
...
%load cifar_kmeans_test
P=PP(1:400,:);
T=TT(1:400,:);
TV.P=PP(401:575,:);
TV.T=TT(401:575,:);
%TV.T=test(:,size(test,2));
%clear test
%SPECTF1 = csvread('mnist_test_norm.csv');
%P=SPECTF1(:,2:size(SPECTF1,2))';
%TV.T=SPECTF1(:,1);
